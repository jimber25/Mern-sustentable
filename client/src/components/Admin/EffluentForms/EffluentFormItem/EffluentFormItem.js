...
export function EffluentFormItem(props) {
  ...
  const onDelete = async () => {
    try {
      ...
      await effluentFormController.deleteEffluentForm(accessToken, effluentForm._id);
      onReload();
      onOpenCloseConfirm();
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
        ...
        <Table.Cell>
          ...
          effluentForm.creator_user.lastname? effluentForm.creator_user.lastname + " " + effluentForm.creator_user.firstname
          : effluentForm.creator_user.email : null}
        </Table.Cell>
        <Table.Cell>
          {isMaster(role) ||
          isAdmin(role) ||
