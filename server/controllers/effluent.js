...
async function deleteEffluent(req, res) {
  try {
      ...
      const { id } = req.params;
      const doc = await Effluent.findById({_id:id});
          ...
          fieldValue.forEach(item => {
            if (item.files && Array.isArray(item.files)) {
              ...
              item.files.forEach(file => {
                const filePath = path.join("uploads/files/effluent", file.uniqueName); // Ruta completa del archivo
      ...
      res.status(400).send({ msg: "Error al eliminar el formulario de Efluente" });
    }
}
...
function uploadFile(req, res) {
  ...
  if (Array.isArray(files)) {
    ...
    files.forEach((file, index) => {
      ...
      const filePath = path.join("uploads/files/effluent", fileName);
...
function getFile(req, res) {
    ...
    const filePath = "./uploads/files/effluent" + fileName;
